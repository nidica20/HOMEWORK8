...
bool PlaceQueen(int column) {
	if (column == NUMBER_COLS) { // Base Condition
		//New solution is stored into the vector since the vector is empty and the solution is not found in an empty vector.
		if (PlacementSolutions.empty())
		{
			PlacementSolutions.push_back(RowPlacement);
		}

		//When comparing all solutions in solutions vector to a new solution, it is added if not already found in the placement solutions vector.
		if (int i = 0; i < PlacementSolutions.size(); i++)
		{
			if (int j = 0; j < RowPlacement.size(); j++)
			{
		...
		for (int row = 0; row < RowPlacement.size(); ++row) {
			...
			//Moves to next row if RowPlacement[row] is not UNASSIGN_VALUE
			if (RowPlacement[row] != UNASSIGN_VALUE)
			{
				continue;
			}

			//Checking if assigning a column queen to this row is valid
			...
			//Recursively calls PlaceQueen to place a queen in the nex column.
			if (PlaceQueen(column+1))
			{
				return true;
			}

			//Unassign the assignment of column's queen to this row and tries the next one
